...
**目标：**
1. 消除重复：配置、i18n、登录门控、管理员名单各收敛为**单一事实来源**。
2. 建立质量闸门（loop）：每次改动自动跑 lint + 精准测试 + 静态扫描，push 时机器兜底。
3. 安全加固：Firestore 规则、`/api` 入参校验、安全响应头、无密钥入仓。
4. 清理死文件与游离文档，仓库干净。
5. 全程渐进、可独立验收，非专业者能看懂每一步在做什么。
...
交付物：
- 安全：Firestore 规则复审（visits/users/bids/meta 权限最小化）；`/api` 各函数入参校验（类型/长度/必填）；新增 Cloudflare `_headers` 安全响应头（CSP、X-Content-Type-Options、Referrer-Policy 等）；确认无密钥进仓库；限流覆盖检查。
- 用户友好：错误提示文案统一（复用 i18n）；无障碍（JS 生成的按钮/输入补 title/alt）；各页移动端自适应过一遍。

验收：安全响应头生效（curl 可见）；`/api` 对畸形入参有防御；各页移动端无溢出。
