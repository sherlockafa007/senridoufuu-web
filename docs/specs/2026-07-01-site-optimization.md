...
## 11. 从既有 BACKLOG 吸收的补充项（2026-07-01 合并）

原 `docs/BACKLOG.md` 的内容已并入本 spec 并删除该文件，避免两份路线图并存打架。归并如下：

**已被本 spec 覆盖（无需重复）：**
- i18n 拆分 → 第二期（共享 `js/shared/i18n.js`）。
- API 入参校验 → 第三期·安全。
- 部署链简化（两跳→一跳）→ 已在会话中评估：需把 Cloudflare Pages 迁到自有账号，工程量大（域名、环境变量迁移），列为**长期**、非近期。

**新吸收（补入对应维度/期）：**
- **工具使用统计**（每次 API 调用记录 tool/uid/timestamp，后台加卡片看哪个功能有人用）→ 可观测性 + 成本（判断哪个工具值得留）。归第三期。
- **Functions 错误日志**（try/catch 写 Firestore `error_logs`，后台可查）→ 稳定 + 可观测性。归第三期。
- **部署验证脚本**（push 后自动 curl 关键 endpoint 比对版本/关键字，失败告警）→ 稳定 + loop 增强。归第三期（或更早，若手动 curl 成为负担）。
- **管理后台整合**（使用统计 / 错误日志 / 爬虫监控 合到一个视图 + 时间筛选）→ 友好 + 维护。归第三期。
- **HTML 模块化**（header/footer/nav 共享模板）→ 维护，是第二期去重的延伸。触发时机：改一个导航项需动 5+ 个文件时。

**⚠️ 统一成本提醒（贯穿实施）：** 工具使用统计、错误日志、限流 `rate_limits`、访问 `visits` 都是"每次操作写一条 Firestore"的高频写入源、且默认只增不减。实施任何一个时都要同时定策略：**采样 / 聚合 / TTL 清理**，避免多个无限膨胀的集合叠加，长期拖高成本与查询延迟。
