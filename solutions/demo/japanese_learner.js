...
const VOCAB = [
  // ── 五段動詞 (Godan / Group 1) ────────────────────────────────
  { id:'v01', kanji:'行く',   furigana:'いく',      romaji:'iku',      pos:'verb_1', zh:'去',        en:'to go' },
  { id:'v02', kanji:'書く',   furigana:'かく',      romaji:'kaku',     pos:'verb_1', zh:'写',        en:'to write' },
  { id:'v03', kanji:'聞く',   furigana:'きく',      romaji:'kiku',     pos:'verb_1', zh:'听/问',     en:'to listen / ask' },
  { id:'v04', kanji:'飲む',   furigana:'のむ',      romaji:'nomu',     pos:'verb_1', zh:'喝',        en:'to drink' },
  { id:'v05', kanji:'読む',   furigana:'よむ',      romaji:'yomu',     pos:'verb_1', zh:'读',        en:'to read' },
  { id:'v06', kanji:'話す',   furigana:'はなす',    romaji:'hanasu',   pos:'verb_1', zh:'说话',      en:'to speak' },
  { id:'v07', kanji:'待つ',   furigana:'まつ',      romaji:'matsu',    pos:'verb_1', zh:'等待',      en:'to wait' },
  { id:'v08', kanji:'買う',   furigana:'かう',      romaji:'kau',      pos:'verb_1', zh:'买',        en:'to buy' },
  { id:'v09', kanji:'帰る',   furigana:'かえる',    romaji:'kaeru',    pos:'verb_1', zh:'回家',      en:'to return home', note:'五段（看似一段）' },
  // ── 一段動詞 (Ichidan / Group 2) ─────────────────────────────
  { id:'v10', kanji:'食べる', furigana:'たべる',    romaji:'taberu',   pos:'verb_2', zh:'吃',        en:'to eat' },
  { id:'v11', kanji:'見る',   furigana:'みる',      romaji:'miru',     pos:'verb_2', zh:'看',        en:'to see / watch' },
  { id:'v12', kanji:'起きる', furigana:'おきる',    romaji:'okiru',    pos:'verb_2', zh:'起床',      en:'to wake up' },
  { id:'v13', kanji:'寝る',   furigana:'ねる',      romaji:'neru',     pos:'verb_2', zh:'睡觉',      en:'to sleep' },
  { id:'v14', kanji:'教える', furigana:'おしえる',  romaji:'oshieru',  pos:'verb_2', zh:'教',        en:'to teach' },
  // ── 不規則動詞 (Irregular / Group 3) ─────────────────────────
  { id:'v15', kanji:'する',   furigana:'する',      romaji:'suru',     pos:'verb_3', zh:'做',        en:'to do' },
  { id:'v16', kanji:'来る',   furigana:'くる',      romaji:'kuru',     pos:'verb_3', zh:'来',        en:'to come' },
  // ── い形容詞 (i-adjective) ────────────────────────────────────
  { id:'a01', kanji:'美しい', furigana:'うつくしい',romaji:'utsukushii',pos:'adj_i', zh:'美丽的',   en:'beautiful' },
  { id:'a02', kanji:'高い',   furigana:'たかい',    romaji:'takai',    pos:'adj_i',  zh:'贵的/高的', en:'expensive / tall' },
  { id:'a03', kanji:'大きい', furigana:'おおきい',  romaji:'ookii',    pos:'adj_i',  zh:'大的',     en:'big / large' },
  { id:'a04', kanji:'難しい', furigana:'むずかしい',romaji:'muzukashii',pos:'adj_i', zh:'难的',     en:'difficult' },
  { id:'a05', kanji:'新しい', furigana:'あたらしい',romaji:'atarashii',pos:'adj_i',  zh:'新的',     en:'new' },
  // ── な形容詞 (na-adjective) ───────────────────────────────────
  { id:'a06', kanji:'元気',   furigana:'げんき',    romaji:'genki',    pos:'adj_na', zh:'精神/健康', en:'healthy / energetic' },
  { id:'a07', kanji:'静か',   furigana:'しずか',    romaji:'shizuka',  pos:'adj_na', zh:'安静的',   en:'quiet' },
  { id:'a08', kanji:'便利',   furigana:'べんり',    romaji:'benri',    pos:'adj_na', zh:'方便的',   en:'convenient' },
  // ── 名詞 (Noun) ───────────────────────────────────────────────
  { id:'n01', kanji:'学校',   furigana:'がっこう',  romaji:'gakkou',   pos:'noun',   zh:'学校',     en:'school' },
  { id:'n02', kanji:'時間',   furigana:'じかん',    romaji:'jikan',    pos:'noun',   zh:'时间',     en:'time' },
];
...
const POS_LABEL = {
  verb_1:'五段動詞', verb_2:'一段動詞', verb_3:'不規則動詞',
  adj_i:'い形容詞',  adj_na:'な形容詞', noun:'名詞',
};
...
function conjugate(w) {
  switch (w.pos) {
    case 'verb_1': return conjugateGodan(w);
    case 'verb_2': return conjugateIchidan(w);
    case 'verb_3': return conjugateIrregular(w);
    case 'adj_i':  return conjugateAdjI(w);
    case 'adj_na': return conjugateAdjNa(w);
    case 'noun':   return conjugateNoun(w);
    default:       return [];
  }
}
...
function filteredVocab() {
  ...
  return VOCAB.filter(w => {
    ...
    const text = !q ||
      w.kanji.includes(q) || w.furigana.includes(q) ||
...
function renderDetail() {
  ...
  if (!w) { el.innerHTML = '<p class="text-gray-400 text-center py-8">请选择一个单词</p>'; return; }
...
function renderQuizModes() {
  ['meaning', 'conjugation'].forEach(m => {
    ...
    btn.className = `py-2.5 rounded-xl text-sm font-medium border-2 transition-all ${
      on ? 'border-blue-500 bg-blue-50 text-blue-700'
...
function renderScoreDots() {
  document.getElementById('score-dots').innerHTML =
    S.quiz.hist.slice(-10).map(ok =>
      `<div class="w-2 h-2 rounded-full ${ok ? 'bg-emerald-400' : 'bg-red-400'}"></div>`
    ).join('');
}
...
function renderScoreText() {
  ...
  if (!total) { el.textContent = '准备好了，开始！'; return; }
...
function renderQuestion() {
  ...
  qCard.className = 'bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-4 text-center min-h-36 flex flex-col items-center justify-center fade-in';
  qCard.innerHTML = `
  ...
  if (answered) {
    ...
    fb.className = '';
    fb.innerHTML = `<div class="rounded-xl p-4 border ${ok
